  ...
  return (
    <div className="py-[60px] h-full px-[20px] mx-auto max-w-[500px]">
      ...
      <div className="mb-[16px]">
        ...
        <span className="text-[14px] mr-[16px]">이미 가입한 계정이 있으세요?</span>
        <Link href={"/signin"}>
      ...
      <div className="mb-[24px]">
        <div className="mb-[15px]">
          ...
          <MemberInput type="email" placeholder="이메일을 입력해주세요" onChange={setEmail} />
          <div className="text-red-600 mt-[3px] ml-[10px] text-[15px]">{emailValidateText}</div>
        </div>
        <div className="mb-[15px]">
          <label className="text-[14px] inline-block w-full">닉네임</label>
          <MemberInput type="text" placeholder="닉네임을 입력해주세요" onChange={setNickName} maxLength={8} />
          <div className="text-red-600 mt-[3px] ml-[10px] text-[15px]">{nickNameValidateText}</div>
        </div>
        <div className="mb-[16px]">
          <label className="text-[14px] inline-block w-full">비밀번호</label>
          <MemberInput type="password" placeholder="비밀번호를 입력해주세요" onChange={setPassword} />
          <div className="text-red-600 mt-[3px] ml-[10px] text-[15px]">{passwordValidateText}</div>
        </div>
      </div>
      ...
      {!isJoinClicked ? (
        <button
          disabled={!(ageCheck && serviceTermCheck && emailValidate && passwordValidate && nickNameValidate)}
          onClick={async (e) => {
        ...
        <div>
          ...
          <div className={!isMobile ? "flex justify-center" : ""}>
            <div className={!isMobile ? "w-[100px]" : "w-full text-center"}>인증번호 입력: </div>
            <input
              className={
                !isMobile
